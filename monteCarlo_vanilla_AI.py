...
Minimax doesn't really work for this game because the branching factor is too high: 
something like 1500.

So analyzing 3 moves ahead is 1500^3 = 3,375,000,000 (3 billion) calculations.
You also need to invent a utility function, some way of analyzing a board state
to decide how good or bad it is, and nobody really knows how to define that for Pai Sho.

I managed to decrease the branching factor to a max of like 300 using 
some heuristics (about the same as 'Go'), but that's still too much to use Minimax.
So MCTS it is.
...
class MonteCarlo:
    ...
    def simulate_better(self, node):
        ...
        while winner == -1:
            ...
            #don't simulate past 40 moves... most games don't go past 30 moves total
            count+=1
            if count > 40: 
                return 0
        return winner
    ...
    def backpropagate(self, node, winner):
        while node != None:
            ...
            #if player wins, increment win count
            player = node.parent.boardState.turnPlayer #the parent node's player is the one that performed this node's move
            if player == winner: 
                node.n_wins += 1
            #if enemy wins, decrement win count
            elif 1 + (player)%2 == winner: 
#                node.n_wins -= 1 #TODO: idk if this is necessary: many implementations only increment the win count and do not decrement when for losses
                pass 
